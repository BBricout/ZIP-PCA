...
Rcpp::List nlopt_optimize_ZIP(
    ...
    // Définir les bornes inférieures pour S
    std::vector<double> lower_bounds(metadata.packed_size, -HUGE_VAL);
    if (config.containsElementNamed("lower_bound_S")) {
        double lower_bound_S = Rcpp::as<double>(config["lower_bound_S"]);
        set_from_r_sexp(metadata.map<S_ID>(lower_bounds.data()), lower_bound_S);
    }
    
        // Appliquer les bornes à l'optimiseur
