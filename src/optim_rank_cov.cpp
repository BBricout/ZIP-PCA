...
// [[Rcpp::export]]
Rcpp::List nlopt_optimize_rank_miss(
    ...
    auto objective_and_grad = [&metadata, &O, &X, &Y, &w, &R](const double * params, double * grad) -> double {
        ...
        metadata.map<B_ID>(grad) = (X.each_col() % w).t() *  (R % (A - Y)) ;
...
// [[Rcpp::export]]
Rcpp::List nlopt_optimize_vestep_rank_miss(
        const Rcpp::List & data  , // List(Y, X, O, w)
    ...
    // Optimize
    auto objective_and_grad = [&metadata, &O, &X, &Y, &R, &w, &B, &C](const double * params, double * grad) -> double {
        const arma::mat M = metadata.map<M_ID>(params);
        const arma::mat S = metadata.map<S_ID>(params);
